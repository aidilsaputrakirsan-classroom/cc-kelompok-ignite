...
class User(Base):
    ...
    )  # Perbaikan: password_hash (sesuai ERD)
...
class Product(Base):
    ...
    category = Column(
        String(50), nullable=False, default="makanan", index=True
    )  # makanan, minuman, snack, dll
    slug = Column(
        String(100), nullable=True, unique=True
    )  # Perbaikan: Tambah slug (sesuai ERD)
    price = Column(Float, nullable=False)
    stock = Column(Integer, nullable=False, default=0)
    image_url = Column(String(255), nullable=True)
    is_active = Column(
        Boolean, default=True
    )  # Perbaikan: is_active (bukan is_available)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class Cart(Base):
    ...
    )  # Perbaikan: Tambah status (sesuai ERD)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
class CartItem(Base):
    ...
    )  # Perbaikan: Sesuai ERD (price_at_time)
...
class Order(Base):
    ...
    )  # Perbaikan: order_code (bukan order_number)
    receipt_name = Column(
        String(100), nullable=False
    )  # Perbaikan: Tambah receipt_name (sesuai ERD)
    recipient_phone = Column(
        String(20), nullable=False
    )  # Perbaikan: recipient_phone (bukan ordering_phone)
    shipping_address = Column(
        Text, nullable=False
    )  # Perbaikan: shipping_address (bukan ordering_address)
...
class Payment(Base):
    ...
    )  # Perbaikan: Tambah paid_at (sesuai ERD)
    verified_by = Column(
        Integer, ForeignKey("users.id"), nullable=True
    )  # Perbaikan: INT (user_id), bukan String
    verified_at = Column(
        DateTime(timezone=True), nullable=True
    )  # Perbaikan: Tambah verified_at (sesuai ERD)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
class Testimonial(Base):
    ...
    )  # Restore: order_id (relasi ke order)
    product_id = Column(
        Integer,
